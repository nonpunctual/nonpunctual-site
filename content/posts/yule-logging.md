...
1) Logging is always a hot topic. "How do I create a log for my script?" "Where should I write log output?" etc. 

Without saying this solution is definitive, the script below has a pattern that I have used over & over. In fact, I paste this block exactly into any script I create that needs logging. 

It uses `tee`, `exec &>` with a named pipe. 

Why? I find this works in all situations to capture all foreground & background processes executed by the script & it creates a dedicated stream / file for the log output regardless of what the script is trying to do.
...
The script below does not do anything remarkable. It removes Network Segments at their endpoint by name. 

This is also something that seems to come up repeatedly. The Jamf API docs could be a bit clearer on this subject. Using the object ID is straightforward but I think having a good example of using name as the identifier should be useful to some of you. Happy Whatever!

**PS.** The script input should just be a single column `.csv` file of single quoted Network Segment names. Put the path to that file in the `network_data_csv` variable at the top of the script.
